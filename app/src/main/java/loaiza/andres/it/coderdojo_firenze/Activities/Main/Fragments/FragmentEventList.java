...
import loaiza.andres.it.coderdojo_firenze.Helpers.EventBriteHelper;
import loaiza.andres.it.coderdojo_firenze.Networking.EventBrite.AsyncBriteEventsRequest;
import loaiza.andres.it.coderdojo_firenze.Networking.EventBrite.EventBriteRequestManager;
import loaiza.andres.it.coderdojo_firenze.Networking.EventBrite.Response.EventBriteEvents;
import loaiza.andres.it.coderdojo_firenze.Networking.EventBrite.RetrofitInterfaces.EventBriteListener;
...
public class FragmentEventList extends Fragment implements EventBriteListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    private void requestEvents(boolean forceUpdate) {
        if (requestManager.getLastEvents() == null || forceUpdate) {
            onRequestStarted();
            new AsyncBriteEventsRequest(this, requestManager).execute(EventBriteRequestManager.TEST_ID); // TODO change with DOJO_ID
        } else {
